// Package server is the orders gRPC transport. Its CreateOrder is where services
// compose: it calls the catalog service over gRPC to reserve stock and snapshot
// prices before storing the order.
package server
...
type Server struct {
	...
	// catalog is the gRPC client to the catalog service. Depending on the generated
	// client interface (not a concrete struct) keeps this testable.
...
func (s *Server) CreateOrder(ctx context.Context, r *ordersv1.CreateOrderRequest) (*ordersv1.Order, error) {
	...
	for _, line := range r.GetItems() {
		...
		// The fan-out. ctx carries the request-id; the client interceptor forwards
		// it, so catalog logs the same id as us.
		res, err := s.catalog.ReserveStock(ctx, &catalogv1.ReserveStockRequest{
			ProductId: line.GetProductId(),
			Quantity:  line.GetQuantity(),
		})
		if err != nil {
			// gRPC propagates the downstream status code across the boundary, so a
			// NotFound / FailedPrecondition / Unavailable from catalog reaches the
			// gateway unchanged — just return it.
			return nil, err
		}
